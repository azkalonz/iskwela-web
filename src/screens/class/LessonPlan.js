...
function LessonPlan(props) {
  const [lessonPlan, setLessonPlan] = useState(sampleActivities);
  ...
  const _handleSort = () => {
    if (sortType === "ASCENDING") {
      setLessonPlan(
        lessonPlan.sort((a, b) => ("" + a.title).localeCompare(b.title))
      );
      setSortType("DESCENDING");
    } else {
      setLessonPlan(
        lessonPlan.sort((a, b) => ("" + b.title).localeCompare(a.title))
      );
              ...
              <RootRef rootRef={provided.innerRef}>
                <List style={getListStyle(snapshot.isDraggingOver)}>
                  {lessonPlan
                    .filter(
                      (l) =>
                        JSON.stringify(l).toLowerCase().indexOf(search) >= 0
                    )
                    .map((item, index) => (
                      <Draggable
                        key={item.id}
