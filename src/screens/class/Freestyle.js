...
function Freestyle(props) {
        ...
        if (data?.length) {
          data = data.map((q) => {
            ...
            if (publishedAssignments?.length) {
              publishedAssignments.forEach((qq) => {
                allData.push({ ...qq, schedule });
              });
            }
          });
      ...
      if (getFiles("activity-materials").getAll("files[]").length) {
        await asyncForEach(
          ...
          async (file) => {
            ...
            let a;
            try {
              a = await FileUpload.upload(
                "/api/assignment/v2/upload/material",
                {
                  body,
  ...
  const _handleUpdateActivityStatus = async (a, s) => {
    ...
    setConfirmed({
      title: stat + " Assignments",
      message: "Are you sure you want to " + stat + " this assignment?",
      yes: async () => {
        setErrors(null);
        setSaving(true);
        setConfirmed(null);
  ...
  const _handleUpdateActivitiesStatus = (a, s, callback = null) => {
    let stat = s ? "Publish" : "Unpublish";
    setConfirmed({
      title: stat + " " + Object.keys(a).length + " Assignments",
      message: "Are you sure to " + stat + " this assignments?",
      yes: async () => {
        setErrors(null);
        setSaving(true);
        setConfirmed(null);
  ...
  const _handleRemoveActivities = (activities, callback = null) => {
    setConfirmed({
      title: "Remove " + Object.keys(activities).length + " Assignments",
      message: "Are you sure to remove this assignments?",
      yes: async () => {
        setErrors(null);
        setSaving(true);
        setConfirmed(null);
  ...
  const _handleRemoveMaterial = (material, index) => {
    setConfirmed({
      ...
      yes: async () => {
        ...
        setConfirmed(null);
        let res = await Api.post(
          "/api/assignment/v2/remove/material/" + material.id,
          {
            body: {
              id: material.id,
            },
